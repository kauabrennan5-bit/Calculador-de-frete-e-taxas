...
import { calcRotas, calcTributos, valorIdealDeclarar, getDescricao, gerarAnalise } from './calculos.js'

// ── COMPONENTES BASE ───────────────────────────────────────────────────────

function Label({ children }) {
  return (
    <div style={{ color: 'var(--accent)', fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', fontFamily: "'Space Mono', monospace", marginBottom: 6 }}>
      {children}
    </div>
  )
}
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: 'var(--muted)', letterSpacing: 3, textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
          {n} — {title}
        </span>
...
function Block({ title, accent = false, children }) {
  return (
    <div style={{
      background: 'var(--surface)',
      border: '1px solid var(--border)',
      borderLeft: `3px solid ${accent ? 'var(--accent)' : 'var(--border)'}`,
      borderRadius: 6,
      padding: '18px 20px',
      marginBottom: 12
    }}>
      <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: 'var(--accent)', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 14 }}>
        ◆ {title}
      </div>
      {children}
    </div>
...
function Card({ label, value, sub, color }) {
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '16px 14px' }}>
      ...
      <div style={{ fontSize: 22, fontWeight: 900, color: color || 'var(--accent)', lineHeight: 1 }}>{value}</div>
...
export default function App() {
  ...
  function calc() {
    ...
    const vol6 = (C * L * A) / 6000
    const vol8 = (C * L * A) / 8000

    const rotas = calcRotas(P, vol6, vol8, CB)
    ...
    setRes({ rotas, melhor, landed, trib, vd, rel, analise, CB, V, P, vol8, desc })
  ...
  const MATERIAIS = [['metal','Metal/Aço/Cromo'],['bronze','Bronze/Cobre'],['vidro','Vidro'],['plastico','Plástico/Acrílico'],['ceramica','Cerâmica'],['madeira','Madeira'],['marmore','Mármore/Pedra'],['misto','Misto']]
  ...
  return (
    <div style={{ maxWidth: 820, margin: '0 auto', padding: '32px 16px 100px' }}>
      ...
      <header style={{ borderBottom: '1px solid var(--border)', paddingBottom: 24, marginBottom: 40 }}>
        <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: 'var(--accent)', letterSpacing: 4, marginBottom: 8 }}>
          IMPORTCALC — B2C
        </div>
        ...
        <p style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, color: 'var(--muted)', marginTop: 10, lineHeight: 1.6 }}>
          Calcule frete, tributos e viabilidade de qualquer produto importado da China.
        </p>
      </header>

      {/* FORM */}
      <Section n="01" title="produto">
        <div style={{ display: 'grid', gap: 14 }}>
          <Field label="Nome do produto">
            <input style={inp('nome')} value={f.nome} onChange={e => s('nome', e.target.value)}
              placeholder="Ex: Mushroom Table Lamp, abajur laranja, haste cromada" />
          </Field>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <Field label="Material principal">
              <select style={inp('material')} value={f.material} onChange={e => s('material', e.target.value)}>
                <option value="">Selecione...</option>
                {MATERIAIS.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </Field>
            <Field label="Categoria">
              <select style={inp('categoria')} value={f.categoria} onChange={e => s('categoria', e.target.value)}>
                <option value="">Selecione...</option>
      ...
      <Section n="02" title="dimensões e peso">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 14 }}>
          <Field label="Comprimento (cm)">
            <input style={inp('comp')} type="number" value={f.comp} onChange={e => s('comp', e.target.value)} placeholder="44" />
          </Field>
          <Field label="Largura (cm)">
            <input style={inp('larg')} type="number" value={f.larg} onChange={e => s('larg', e.target.value)} placeholder="28" />
          </Field>
          <Field label="Altura (cm)">
            <input style={inp('alt')} type="number" value={f.alt} onChange={e => s('alt', e.target.value)} placeholder="22" />
          </Field>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <Field label="Peso real (gramas)">
            <input style={inp('peso')} type="number" value={f.peso} onChange={e => s('peso', e.target.value)} placeholder="1467" />
          </Field>
          <Field label="Pode desmontar?">
            <select style={inp('desmonta')} value={f.desmonta} onChange={e => s('desmonta', e.target.value)}>
      ...
      <Section n="03" title="valor e logística">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
          <Field label="Valor pago (USD $)">
            <input style={inp('valor')} type="number" step="0.01" value={f.valor} onChange={e => s('valor', e.target.value)} placeholder="19.30" />
          </Field>
          <Field label="Câmbio USD/BRL">
            <input style={inp('cambio')} type="number" step="0.01" value={f.cambio} onChange={e => s('cambio', e.target.value)} />
          </Field>
          <Field label="Tem bateria?">
            <select style={inp('bateria')} value={f.bateria} onChange={e => s('bateria', e.target.value)}>
              <option value="nao">Não</option>
              <option value="interna">Bateria interna</option>
              <option value="externa">Bateria removível</option>
            </select>
          </Field>
        </div>
      </Section>

      <Section n="04" title="contexto (opcional)">
        <Field label="Detalhes adicionais">
          <textarea style={{ ...inp('ctx'), minHeight: 80, resize: 'vertical' }}
            value={f.contexto} onChange={e => s('contexto', e.target.value)}
            placeholder="Ex: controle remoto, cabo USB, embalagem original do fabricante..." />
        </Field>
      </Section>

      <button onClick={calc} style={{
        width: '100%', padding: '16px', background: 'var(--accent)', color: '#000',
        border: 'none', borderRadius: 4, fontSize: 15, fontWeight: 800,
        letterSpacing: 2, textTransform: 'uppercase', cursor: 'pointer',
        transition: 'all 0.15s', marginTop: 4
      }}
        onMouseEnter={e => e.target.style.background = '#ffd060'}
      ...
      {res && (
        <div id="resultado" className="fade-up" style={{ marginTop: 52 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 14 }}>
            ...
            <Card label="Frete estimado" value={`R$${res.melhor.freteBRL.toFixed(0)}`} sub={res.melhor.nome} />
            <Card label="Landed cost" value={`R$${res.landed.toFixed(0)}`} sub="produto + frete + tributos" color="var(--red)" />
          </div>

          {/* Declaração */}
          <Block title="declaração aduaneira recomendada" accent>
          ...
          {/* Rotas */}
          <Block title="comparativo de rotas" accent>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 380 }}>
                <thead>
                  <tr>
                    {['Rota', 'Peso fat.', 'Frete est.', 'Prazo', ''].map(h => (
                  ...
                  {res.rotas.map(r => {
                    ...
                    return (
                      <tr key={r.nome} style={{ background: b ? 'rgba(240,192,64,0.04)' : 'transparent' }}>
                        ...
                        <td style={{ padding: '9px 10px', color: b ? 'var(--text)' : 'var(--muted)' }}>R${r.freteBRL.toFixed(0)}</td>
                        <td style={{ padding: '9px 10px', color: b ? 'var(--text)' : 'var(--muted)' }}>{r.dias}d</td>
                        <td style={{ padding: '9px 10px' }}>
                          {b && <span style={{ background: 'rgba(240,192,64,0.15)', color: 'var(--accent)', padding: '2px 10px', borderRadius: 3, fontSize: 10, fontFamily: "'Space Mono', monospace", fontWeight: 700 }}>MELHOR</span>}
                        </td>
                      </tr>
                    )
                  })}
          ...
          {/* Análise */}
          <Block title="análise personalizada" accent>
            <div style={{ display: 'grid', gap: 16 }}>
              {res.analise.map((sec, i) => (
                <div key={i} style={{ borderBottom: i < res.analise.length - 1 ? '1px solid var(--border)' : 'none', paddingBottom: i < res.analise.length - 1 ? 16 : 0 }}>
                  ...
                  {sec.linhas.map((l, j) => (
                    <div key={j} style={{ fontSize: 13, color: '#888', lineHeight: 1.7, paddingLeft: 4 }}>{l}</div>
                  ))}
                </div>
              ))}
            </div>
